function [SPM] = spm_spm_WB(SPM,data1,Cm,xyz)
% [Re]ML Estimation of a General Linear Model
% FORMAT [SPM] = spm_spm_WB(SPM)
%
...
%                     spm_spm_WB will set this to whiten the data and render
...
%                     If not specified spm_spm_WB will compute this using
...
% Optional Argument:
%
% Use preloaded data (added by DGM on 3/26/2013):
%   data1 should be either: (1) a file with 2 variables - Y and Cm; or (2)
%       the variable Y with the timeseries data
%   data2 should be the Cm variable data.
%__________________________________________________________________________
%
% spm_spm_WB is the heart of the SPM package. Given image files and a
...
% component) is estimated, even if V is not i.i.d.  This means spm_spm_WB
...
% re-entered into spm_spm_WB to make an inference (SPM) at the next
...
%--------------------------------------------------------------------------
%
% The output of spm_spm_WB takes the form of an SPM.mat file of the analysis
...
%                           ----------------
%
% The following SPM.fields are set by spm_spm_WB (unless specified)
%
...
% $Id: spm_spm_WB.m 4515 2011-10-07 18:26:00Z guillaume $
...
if isfield(xX,'W')
    ...
    VM    = struct('fname',  'mask.img',...
                   ...
                   'mat',    M,...
                   'pinfo',  [1 0 0]',...
                   'descrip','spm_spm_WB:resultant analysis mask');
    ...
    for i = 1:nBeta
        ...
        Vbeta(i).descrip = sprintf('spm_spm_WB:beta (%04d) - %s',i,xX.name{i});
    end
    ...
    VResMS = struct('fname',    'ResMS.img',...
        ...
        'mat',      M,...
        'pinfo',    [1 0 0]',...
        'descrip',  'spm_spm_WB:Residual sum-of-squares');
    ...
    VResI(1:nSres) = deal(struct(...
        ...
        'mat',      M,...
        'pinfo',    [1 0 0]',...
        'descrip',  'spm_spm_WB:StandardisedResiduals'));
 
    for i = 1:nSres
        VResI(i).fname   = sprintf('ResI_%04d.img', i);
        VResI(i).descrip = sprintf('spm_spm_WB:ResI (%04d)', i);
    end
...
%Do not loop over planes. Do all planes at once.
%for z = 1:nbz:zdim                       %-loop over planes (2D or 3D data)
 
    % current plane-specific parameters
    %----------------------------------------------------------------------
    CrPl    = 1:1:zdim;                  %min(z+nbz-1,zdim);       %-plane list
    ...
    %Do all voxels at once.
    %for bch = 1:nbch                     %-loop over blocks
    if exist('data1','var') && ischar(data1) && exist(data1,'file')
        load(data1)
    end
    if exist('Y','var') && exist('Cm','var') && exist('xyz','var')
    else
        ...
        %        z,zdim,bch,nbch);
        %else
        str = sprintf('Planes %3d-%-3d/%-3d',1,CrPl(end),zdim); % replace z with 1
        %end
        %if z == 1 && bch == 1
        %    str2 = '';
        %else
        ...
        %------------------------------------------------------------------
        Y     = zeros(nScan,nVox);
        Y(:,Cm) = spm_get_data(VY,xyz(:,Cm),false); % faster to read in the data
        
        % Load data in mask
        %------------------------------------------------------------------
        Cm(Cm)   = all(Y(:,Cm) > repmat(xM.TH(:),1,sum(Cm)),1);         %-Threshold (& NaN) mask
        for ii=1:nScan
            if xM.I && ~YNaNrep && xM.TH(ii) < 0    %-Use implicit mask
            Cm(Cm) = abs(Y(ii,Cm)) > eps;
            end
        end
        %for i = 1:nScan
        %
        ...
        if exist('data1','var') && ischar(data1)
            save(data1,'Y','Cm','xyz')
        else
            save('datamatrix.mat','Y','Cm','xyz')
        end
    end
        ...
        Q                  = [Q find(Cm)];     %-InMask XYZ voxel indices % Cm InMask XYZ voxels, find gives indices
    ...
    %commented out due to var not existing
...
if ~isfield(xVi,'V')
    ...
    if ~isfield(xX,'W')
        ...
        clearvars -except Y Cm xyz
        load('SPM.mat');
        SPM = spm_spm_WB(SPM,Y,Cm,xyz);
        return
    end
end
...
try
    ...
    VRpv = SPM.xVol.VRpv;
    R    = SPM.xVol.R;
    if ~isempty(VRpv) && ~exist(VRpv.fname,'file')
